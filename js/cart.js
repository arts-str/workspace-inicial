...
function removeFromCart(prodID) {
  ...
  const deleteElement = globalCart.find(
    (p) => p.product.id === Number.parseInt(prodID, 10)
  );
  if (deleteElement) {
...
function addOneInCart(prodID) {
  ...
  const updateElement = globalCart.find(
    (p) => p.product.id === Number.parseInt(prodID, 10)
  );
...
function addEventsToCards() {
  ...
  for (const container of titleContainer) {
    ...
    borrarBttn.onclick = (e) => {
      const card =
        e.target.parentElement.parentElement.parentElement.parentElement; //Tantos parent element para devolver la tarjeta
  ...
  for (const fieldset of inputs) {
    ...
    const prodID =
      fieldset.parentElement.parentElement.parentElement.children[0].children[1]
        .innerHTML;
    ...
    const sumProductPrice =
      fieldset.parentElement.parentElement.parentElement.parentElement
    ...
    addBttn.onclick = () => {
      ...
      if (current < max) {
        //Si no es mayor a la cantidad maxima de productos
...
function updateProductPrices(unitPrice, input, sumProductPrice) {
  ...
  const match = new RegExp(/(\d{1,3}(?:,\d{3})*\.\d+)/).exec(
    originalPrice
  )?.[1];
  if (match) {
...
function addCard(product, value) {
  ...
  const price = product.cost.toLocaleString("en-US", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  }); //Formatear el precio
  ...
  return `
        <div class="list-item-container">
            <div class="img-contenido cursor-active">
...
function updateProductNumber() {
  ...
  // Deshabilitar botón de comprar y botones de envío
  // Mostrar mensaje de "¡Tu carrito está vacío!" cuando la suma de productos sea 0
  if (sum == 0) {
