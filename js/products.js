...
function productResultCount() {
  ...
  ).innerHTML = `Cantidad de resultados: ${currentProductArray.length}`;
}
...
function setProductTitle(name) {
  document.getElementById(
    "product-title"
  ).innerHTML = `Resultados para ${name}`;
}
  ...
  for (let product of currentProductArray) {
    htmlContentToAppend += `
            <div onclick="setProductID(${
              product.id
            })" class="list-item-container cursor-active">
                ...
                <div class="contenido">
                    ...
                    <div class="monedas">
                        ...
                        <div class="resaltado">
                        <p class="precio"> ${product.cost.toLocaleString(
                          "en-US",
                          {
...
document.addEventListener("DOMContentLoaded", (e) => {
  ...
  document
    ...
    .addEventListener("click", function () {
      ...
      currentProductArray = orderByMinMax(
        currentProductArray,
        ORDER_COST,
        minNum,
        maxNum
 ...
 * Auxiliares
 */
function orderAscString(array, key) {
  // Orden ascendente por campo de string
  array.sort((a, b) => {
    return a[key].localeCompare(b[key]);
  });
}

function orderDescString(array, key) {
  // Orden descendente por campo de string
  array.sort((a, b) => {
    return b[key].localeCompare(a[key]);
  });
}

function orderAscNumber(array, key) {
  // Orden ascendente por campo de number
  array.sort((a, b) => {
    return b[key] - a[key];
  });
}

function orderDescNumber(array, key) {
  // Orden descendente por campo de number
  array.sort((a, b) => {
    return a[key] - b[key];
  });
}
...
function buscadorTR() {
  ...
  let dropdownContainer = document.body.appendChild(
  ...
  let busqueda = (valor) => {
    ...
    currentProductArray = valor
      ? all.filter((productos) =>
          (productos.name + productos.description).toLowerCase().includes(valor)
        )
      : all;
    showProductsList();
    ...
    if (!valor || !currentProductArray.length) {
  ...
  buscador.onfocus = (e) => {
    if (dropdownContainer.innerHTML) busqueda(e.target.value);
  };
  // Oculta el dropdown al perder el foco
  buscador.onblur = (_) =>
    setTimeout((_) => (dropdownContainer.style.display = "none"), 150);
  // Reposiciona el dropdown al hacer scroll o cambiar tamaño de ventana
  ["scroll", "resize"].forEach((ev) =>
    window.addEventListener(ev, (_) => {
      if (dropdownContainer.style.display === "block") busqueda(buscador.value);
    })
  );
